...
// vendor/tmux/session.c:41  int session_cmp(struct session *s1, struct session *s2)
pub fn session_cmp(s1: &session, s2: &session) -> cmp::Ordering {
    s1.name.cmp(&s2.name)
}

// vendor/tmux/session.c:48  int session_group_cmp(struct session_group *s1, struct session_group *s2)
pub fn session_group_cmp(s1: &session_group, s2: &session_group) -> cmp::Ordering {
    s1.name.cmp(&s2.name)
}

// vendor/tmux/session.c:59  int session_alive(struct session *s)
pub unsafe fn session_alive(s: *mut session) -> bool {
    unsafe { rb_foreach(&raw mut SESSIONS).any(|s_loop| s_loop.as_ptr() == s) }
}

/// Find session by name.
// vendor/tmux/session.c:72  struct session *session_find(const char *name)
...
// vendor/tmux/session.c:82  struct session *session_find_by_id_str(const char *s)
...
/// Find session by id.
// vendor/tmux/session.c:98  struct session *session_find_by_id(u_int id)
...
// vendor/tmux/session.c:111  struct session *session_create(const char *prefix, const char *name, const char *cwd, struct environ *env, struct options *oo, struct termios *tio)
...
// vendor/tmux/session.c:161  void session_add_ref(struct session *s, const char *from)
...
// vendor/tmux/session.c:169  void session_remove_ref(struct session *s, const char *from)
...
/// Free session.
// vendor/tmux/session.c:180  static void session_free(__unused int fd, __unused short events, void *arg)
...
/// Destroy a session.
// vendor/tmux/session.c:197  void session_destroy(struct session *s, int notify, const char *from)
...
// vendor/tmux/session.c:233  static void session_lock_timer(__unused int fd, __unused short events, void *arg)
...
// vendor/tmux/session.c:249  void session_update_activity(struct session *s, struct timeval *from)
...
// vendor/tmux/session.c:277  struct session *session_next_session(struct session *s, struct sort_criteria *sort_crit)
...
// vendor/tmux/session.c:300  struct session *session_previous_session(struct session *s, struct sort_criteria *sort_crit)
...
// vendor/tmux/session.c:323  struct winlink *session_attach(struct session *s, struct window *w, int idx, char **cause)
...
// vendor/tmux/session.c:341  int session_detach(struct session *s, struct winlink *wl)
...
// vendor/tmux/session.c:362  int session_has(struct session *s, struct window *w)
...
// vendor/tmux/session.c:378  int session_is_linked(struct session *s, struct window *w)
...
// vendor/tmux/session.c:388  static struct winlink *session_next_alert(struct winlink *wl)
...
// vendor/tmux/session.c:400  int session_next(struct session *s, int alert)
...
// vendor/tmux/session.c:419  static struct winlink *session_previous_alert(struct winlink *wl)
...
// vendor/tmux/session.c:431  int session_previous(struct session *s, int alert)
...
// vendor/tmux/session.c:451  int session_select(struct session *s, int idx)
...
// vendor/tmux/session.c:461  int session_last(struct session *s)
...
// vendor/tmux/session.c:476  int session_set_current(struct session *s, struct winlink *wl)
...
// vendor/tmux/session.c:502  struct session_group *session_group_contains(struct session *target)
...
// vendor/tmux/session.c:518  struct session_group *session_group_find(const char *name)
...
// vendor/tmux/session.c:528  struct session_group *session_group_new(const char *name)
...
// vendor/tmux/session.c:545  void session_group_add(struct session_group *sg, struct session *s)
...
// vendor/tmux/session.c:553  static void session_group_remove(struct session *s)
...
// vendor/tmux/session.c:569  u_int session_group_count(struct session_group *sg)
...
// vendor/tmux/session.c:582  u_int session_group_attached_count(struct session_group *sg)
...
// vendor/tmux/session.c:595  void session_group_synchronize_to(struct session *s)
...
// vendor/tmux/session.c:614  void session_group_synchronize_from(struct session *target)
...
// stack and alerts.
// vendor/tmux/session.c:634  static void session_group_synchronize1(struct session *target, struct session *s)
...
// vendor/tmux/session.c:693  void session_renumber_windows(struct session *s)
