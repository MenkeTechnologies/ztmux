...
pub static OPTIONS_TABLE: [options_table_entry; 248] = [
    options_table_entry {
        name: "backspace",
...
mod tests {
    ...
    // Option lookup (options.c) is a linear scan that returns the *first* entry
    // whose name matches, so a duplicate name would silently shadow the second.
    // Every entry must also be named.
    #[test]
    ...
    // options_default_to_string (options.c) renders a CHOICE default with
    // `choices[default_num]`, so every CHOICE entry must have a non-empty
    // choices list and a default_num that is a valid index into it — otherwise
    // that lookup panics at runtime. Conversely, non-CHOICE entries carry no
    // choices.
    #[test]
    ...
    // NUMBER options must have a sane range (minimum <= maximum) and a default
    ...
    // Helper: look up a table entry by exact name (linear scan, like options.c).
    ...
    // Cross-check the buffer-limit entry against vendor/tmux/options-table.c:291
    // (NUMBER, server scope, minimum 1, maximum INT_MAX, default 50).
    #[test]
    ...
    // Cross-check the status entry against vendor/tmux/options-table.c:1019
    // (CHOICE, session scope, default index 1 == "on").
    #[test]
    fn test_entry_status_matches_c() {
        ...
        // Default renders as choices[1].
        assert_eq!(oe.choices[oe.default_num as usize], "on");
        assert_eq!(oe.choices, &OPTIONS_TABLE_STATUS_LIST);
    }

    // mode-keys and status-keys are emacs/vi CHOICE options defaulting to emacs
    // (index 0). vendor/tmux/options-table.c:1456 and :1073.
    #[test]
    fn test_entry_mode_and_status_keys() {
        ...
        // mode-keys is per-window, status-keys is per-session.
        assert_eq!(table_get("mode-keys").scope, OPTIONS_TABLE_WINDOW);
        assert_eq!(table_get("status-keys").scope, OPTIONS_TABLE_SESSION);
    }

    // vendor/tmux/options-table.c:1569 declares pane-border-status scope as
    // OPTIONS_TABLE_WINDOW|OPTIONS_TABLE_PANE; the port now sets both bits.
    #[test]
    ...
    // Every IS_STYLE entry must be a STRING option — options_from_string_check
    // (options.c:1033) style-parses the value, which only makes sense for
    // strings.
    #[test]
    ...
    // Every IS_HOOK entry is a COMMAND array (the hook macros in options_table.rs
    // build OPTIONS_TABLE_COMMAND | IS_ARRAY | IS_HOOK, matching the
    // OPTIONS_TABLE_HOOK/OPTIONS_TABLE_PANE_HOOK macros in
    // vendor/tmux/options-table.c).
    #[test]
    ...
    // The `unit` label is only meaningful for NUMBER options (it is shown next to
    // numeric values). No non-NUMBER entry should carry one.
    #[test]
    ...
    // OPTIONS_OTHER_NAMES (options-table.c options_other_names[]) maps a legacy
    // alias to a canonical option. The canonical target must exist in the table,
    // and the alias itself must NOT be a real entry (otherwise the mapping would
    // shadow a live option).
    #[test]
    fn test_other_names_targets_resolve() {
        ...
        // Both name-map tables must agree.
    ...
    // Every scope is a combination of the four defined scope bits (server,
    // session, window, pane) — options.c:2615 — with no stray bits set.
    #[test]
    ...
    // Each CHOICE option's choices list has no empty and no duplicate strings —
    // options_find_choice (options.c:1088) does a linear streq scan, so a
    // duplicate would be unreachable and an empty string would match "".
    #[test]
    ...
    // Array options are limited to the value types tmux stores element-wise:
    // STRING, COMMAND (hooks) and COLOUR (pane-colours). This documents the
    // actual set so a future array of an unexpected type is caught.
    #[test]
