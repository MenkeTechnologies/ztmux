...
const LIST_CLIENTS_FIELDS: &[Field] = &[
    Field { key: "name", fmt: c!("#{client_name}"), kind: FieldKind::Str },
    Field { key: "tty", fmt: c!("#{client_tty}"), kind: FieldKind::Str },
    Field { key: "session", fmt: c!("#{session_name}"), kind: FieldKind::Str },
    Field { key: "width", fmt: c!("#{client_width}"), kind: FieldKind::Int },
    Field { key: "height", fmt: c!("#{client_height}"), kind: FieldKind::Int },
    Field { key: "termname", fmt: c!("#{client_termname}"), kind: FieldKind::Str },
    Field { key: "pid", fmt: c!("#{client_pid}"), kind: FieldKind::Int },
];

/// C `vendor/tmux/cmd-list-clients.c:54`: `static enum cmd_retval cmd_list_clients_exec(struct cmd *self, struct cmdq_item *item)`
