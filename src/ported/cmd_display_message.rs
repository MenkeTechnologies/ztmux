...
/// C `vendor/tmux/cmd-display-message.c:53`: `static void cmd_display_message_each(const char *key, const char *value, void *arg)`
...
/// C `vendor/tmux/cmd-display-message.c:61`: `static enum cmd_retval cmd_display_message_exec(struct cmd *self, struct cmdq_item *item)`
unsafe fn cmd_display_message_exec(self_: *mut cmd, item: *mut cmdq_item) -> cmd_retval {
    unsafe {
        ...
        if cmdq_get_client(item).is_null() {
            ...
            status_message_set!(tc, delay as i32, 0, nflag, cflag as i32, "{}", _s(msg));
        }
        free_(msg);
