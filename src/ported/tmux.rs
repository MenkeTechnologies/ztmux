...
pub unsafe fn tmux_main(mut argc: i32, mut argv: *mut *mut u8, _env: *mut *mut u8) {
    ...
    unsafe {
        ...
        */
        ...
        // `extensions::socket`), and from make_label ("default" under the
        ...
        if path.is_null() && label.is_null() {
            path = crate::extensions::socket::socket_from_environment();
        }
        if path.is_null() {
        ...
        if argc >= 1 && !(*argv).is_null() {
            ...
            // Every extension verb dispatches here; the list is EXTENSION_COMMANDS
            // itself, so the CLI dispatch, the command-prompt completion and the
            // `verbs` listing cannot drift apart.
            if crate::extensions::EXTENSION_COMMANDS
                .iter()
                .any(|name| name.as_bytes() == cmd)
            {
                ...
                let code = match cmd {
                    ...
                    b"shadow" => crate::extensions::shadow::run(&sock),
                    b"resurrect" => crate::extensions::resurrect::run(&sock),
                    ...
                    b"verbs" => crate::extensions::verbs::run(&sock),
                    b"banner" => crate::extensions::banner::run(&sock),
