...
unsafe fn window_customize_get_tag(
    ...
    unsafe {
        if let Some(oe) = NonNull::new(oe.cast_mut()) {
            ...
            (2u64 << 62) | (offset << 32) | ((((idx as i64) + 1) as u64) << 1) | 1
        } else {
            o.addr() as u64
        }
    }
...
unsafe fn window_customize_build_keys(
    ...
    unsafe {
        ...
        let title: *mut u8 = format_nul!("Key Table - {}", _s((*kt).name_ptr()));
        let top = mode_tree_add(
            (*data).data,
            null_mut(),
            null_mut(),
        ...
        while !bd.is_null() {
            ...
            (*item).table = xstrdup((*kt).name_ptr()).as_ptr();
            (*item).key = (*bd).key;
    ...
    unsafe {
        ...
        if !oe.is_null() && (*oe).flags & OPTIONS_TABLE_IS_ARRAY != 0 {
            ...
            if let Err(err) = options_array_set(o, idx as u32, Some(cstr_to_str(s)), false) {
                ...
                status_message_set!(c, -1, 1, false, 0, "{err_msg}");
            }
...
pub unsafe fn window_customize_unset_key(
    ...
    unsafe {
        ...
        key_bindings_remove((*kt).name_ptr(), (*bd).key);
    }
}
...
pub unsafe fn window_customize_reset_key(
    ...
    unsafe {
        ...
        key_bindings_reset((*kt).name_ptr(), (*bd).key);
    }
}
...
mod tests {
    ...
    /// Regression for the `idx == -1` tag overflow. C `window-customize.c:122`
    /// computes `(idx + 1) << 1` in signed space, so `idx == -1` yields `0`.
    /// The Rust must not overflow (`-1i32 as u64` = `u64::MAX`, `+1` panics in
    /// dev builds). `oe = &OPTIONS_TABLE[0]` gives table offset `0`.
    #[test]
    fn get_tag_idx_minus_one_no_overflow() {
        unsafe {
            let oe = (&raw const OPTIONS_TABLE[0]).cast::<options_table_entry>();

            // offset = 0, (idx + 1) << 1 = 0  =>  (2 << 62) | 1
            let tag = window_customize_get_tag(null_mut(), -1, oe);
            assert_eq!(tag, (2u64 << 62) | 1);

            // Normal (non-negative) index path stays correct: (5 + 1) << 1 = 12.
            let tag5 = window_customize_get_tag(null_mut(), 5, oe);
            assert_eq!(tag5, (2u64 << 62) | (6u64 << 1) | 1);
        }
    }
}
