...
/// C `vendor/tmux/screen-redraw.c:1011`: `void redraw_invalidate_scene(struct window *w)`
///
/// Bumps the window's redraw-scene generation so a cached scene is rebuilt.
...
pub unsafe fn screen_redraw_draw_panes(ctx: *mut screen_redraw_ctx) {
    unsafe {
        ...
        // Tiled panes first, in pane-list order.
        for wp in tailq_foreach::<_, discr_entry>(&raw mut (*w).panes).map(NonNull::as_ptr) {
            if window_pane_is_floating(wp) == 0 && window_pane_visible(wp) {
                screen_redraw_draw_pane(ctx, wp);
            }
        }

        // Floating panes on top. z_index has the newest at the head, so iterate
        // in reverse (oldest first) to paint the newest floating pane last —
        // the painter's-algorithm draw order gives correct occlusion for free.
        for wp in
...
pub unsafe fn screen_redraw_draw_pane(ctx: *mut screen_redraw_ctx, wp: *mut window_pane) {
    unsafe {
        ...
        // Floating panes get a border box drawn around their content. In the
        // direct-draw model the floating pass paints this on top of the tiled
        // layout (and any lower floating pane), so no occlusion bookkeeping is
        // needed. Vendored builds the same box in its scene cache
        ...
        if window_pane_is_floating(wp) != 0 && (*ctx).pane_lines != pane_lines::PANE_LINES_NONE {
            ...
            memcpy__(&raw mut base, &raw const GRID_DEFAULT_CELL);
            let sty = if std::ptr::eq(wp, (*w).active) {
                c!("pane-active-border-style")
            } else {
                c!("pane-border-style")
            };
            style_add(&raw mut base, oo, sty, ft);
            format_free(ft);

            let statustop_adj = if (*ctx).statustop != 0 {
                (*ctx).statuslines
            } else {
                0
            };
            ...
            for by in topb..=botb {
                for bx in left..=right {
                    if by != topb && by != botb && bx != left && bx != right {
                        continue; // interior — not a border cell
                    }
                    if bx < 0 || by < 0 || bx >= (*w).sx as i32 || by >= (*w).sy as i32 {
                        continue;
                    }
                    let (xu, yu) = (bx as u32, by as u32);
                    if xu < (*ctx).ox
                        || xu >= (*ctx).ox + (*ctx).sx
                        || yu < (*ctx).oy
                        || yu >= (*ctx).oy + (*ctx).sy
                    ...
                    let ct = if bx == left && by == topb {
                        cell_type::CELL_TOPLEFT
                    } else if bx == right && by == topb {
                        cell_type::CELL_TOPRIGHT
                    } else if bx == left && by == botb {
                        cell_type::CELL_BOTTOMLEFT
                    } else if bx == right && by == botb {
                        cell_type::CELL_BOTTOMRIGHT
                    } else if by == topb || by == botb {
                        cell_type::CELL_LEFTRIGHT
                    } else {
                        cell_type::CELL_TOPBOTTOM
                    };
                    let mut gc: grid_cell = zeroed();
                    memcpy__(&raw mut gc, &raw const base);
                    screen_redraw_border_set(w, wp, (*ctx).pane_lines, ct, &raw mut gc);
                    tty_cursor(tty, xu - (*ctx).ox, statustop_adj + (yu - (*ctx).oy));
                    tty_cell(tty, &raw mut gc, &GRID_DEFAULT_CELL, null_mut(), null_mut());
                }
            }
        }
