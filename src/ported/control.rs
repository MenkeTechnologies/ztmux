...
mod tests {
    ...
    #[test]
    fn test_control_pane_cmp() {
        unsafe {
            let mut a: control_pane = zeroed();
            let mut b: control_pane = zeroed();
            a.pane = 1;
            b.pane = 2;
            assert_eq!(control_pane_cmp(&a, &b), cmp::Ordering::Less);
            assert_eq!(control_pane_cmp(&b, &a), cmp::Ordering::Greater);
            b.pane = 1;
            assert_eq!(control_pane_cmp(&a, &b), cmp::Ordering::Equal);
        }
    }
