...
# The ztmux port lives under src/ported/ (recursively: src/ported/ +
# src/ported/compat/). src/extensions/ holds original, non-port code (the
# dashboard, switch, and structured-output features) and is intentionally out
# of scope here — it has no C counterpart, exactly as the anti-drift gate
# treats it. The crate roots src/lib.rs and src/main.rs are glue, not ports.
RS_DIRS = [ROOT / "src" / "ported"]
