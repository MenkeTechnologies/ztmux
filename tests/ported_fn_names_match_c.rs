...
fn ported_fns_match_c_source() {
    ...
    for f in &rust_files {
        let rel = f
            ...
            .to_string_lossy()
            .to_string();
        // `src/extensions/` holds original ztmux features (dashboard, etc.) that
        // are deliberately NOT ports of tmux, so they are exempt from C-name
        // fidelity — the same way `mod tests` blocks are skipped.
        if rel.replace('\\', "/").contains("src/extensions/") {
            continue;
        }
        let src = match fs::read_to_string(f) {
            Ok(s) => s,
